...
describe('This is first test suite, Kaspar Raun', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        // 14.06.2023
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="firstName"]').type('Simmo')
        cy.get('input[name="lastName"]').type('Simmosson')
        cy.get('input[name="password"]').type('SalaS6na')
        cy.get('[name="confirm"]').type('SalaS6na')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        // next 2 lines check exactly the same, but using different approach
        cy.get('#success_message').should('not.be.visible')
        cy.get('#success_message').should('not.have.css', 'display', 'block')
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('[data-testid="phoneNumberTestId"]').type('qwertyuiop')
